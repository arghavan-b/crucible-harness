...
def test_diagnose_cuda_and_oom() -> None:
    assert (
        RuleDiagnoser().diagnose(Symptom("s", 1, "CUDA error: no kernel image"))[0].cause
        is FailureCause.CUDA_DRIVER_MISMATCH
    )
    assert (
        RuleDiagnoser().diagnose(Symptom("s", 1, "RuntimeError: CUDA out of memory"))[0].cause
        is FailureCause.OUT_OF_MEMORY
    )


def test_diagnose_unknown() -> None:
    assert (
        RuleDiagnoser().diagnose(Symptom("s", 1, "weird nonspecific error"))[0].cause
        is FailureCause.UNKNOWN
    )
    ...
    lib = PlaybookLibrary(
        [
            Playbook(
                ...
                repair=RepairAction(command="true"),
...
def test_recovery_fixes_a_failing_step(tmp_path) -> None:
    ...
    assert [capture.context.attempt for capture in step.command_captures] == [0, 0, 1]
    assert [capture.context.role for capture in step.command_captures] == [
        "scientific_action",
        "recovery_action",
        "scientific_action",
    ]
    assert run.capture_summary.capture_count == 3
    events = ex.recorder.events(run.trace_id)
    recovering_index = next(
        index
        for index, event in enumerate(events)
        if event["kind"] == "step_state" and event["payload"]["state"] == "RECOVERING"
    )
    repair_capture_index = next(
        index
        for index, event in enumerate(events)
        if event["kind"] == "command_capture"
        and event["payload"]["context"]["role"] == "recovery_action"
    )
    assert recovering_index < repair_capture_index
...
def test_failed_repair_stops_without_retrying_scientific_action(tmp_path) -> None:
    library = PlaybookLibrary(
        [
            Playbook(
                playbook_id="failed_repair",
                cause=FailureCause.MISSING_DEPENDENCY,
                repair=RepairAction(command="false"),
            )
        ]
    )
    run = _executor(tmp_path, RecoveryEngine(library)).execute(_import_plan(), spec=None)
    ...
    assert [capture.context.attempt for capture in step.command_captures] == [0, 0]
    assert step.repairs[0].applied is False
    assert step.failure_reason == "recovery action exited nonzero: exit_code=1"
